...
function getDayStyles(code, {
  ...
  const dayStateStyle = { // highlighted date
...
function getEdgeDayBeforeStyle(code, firstChild, peekNextMonth) {
  ...
  const lastChildStyle = firstChild ? {} : {
    content: 'none'
  };
...
export const StyledDay = styled('div', props => {
  ...
  return {
    ...
    ':first-child': { ...($range ? {
        ':before': { ...getEdgeDayBeforeStyle(code, true, $peekNextMonth)
        }
      } : {})
    },
    ':last-child': { ...($range ? {
        ':before': { // ...getEdgeBeforeStyles(props, false),
