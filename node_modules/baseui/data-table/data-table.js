...
var InnerTableElement = React.forwardRef(function (props, ref) {
  ...
  return React.createElement("div", {
    ...
    style: props.style
  }, React.createElement(Headers, null), React.Children.toArray(props.children).length <= ctx.columns.length ? React.createElement("div", {
    className: css(_objectSpread2({}, theme.typography.font100, {
      marginTop: theme.sizing.scale600,
      marginLeft: theme.sizing.scale600
    }))
  }, "No rows match the filter criteria defined. Please remove one or more filters to view more data.") : props.children, ctx.rowActions && Boolean(ctx.rowActions.length) && ctx.rowHighlightIndex > 0 && !ctx.isScrollingX && React.createElement("div", {
    style: {
      alignItems: 'center',
...
function Unstable_DataTable(props) {
  ...
  var normalizedWidths = React.useMemo(function () {
    ...
    var resizedWidths = measuredWidths.map(function (w, i) {
      return w + resizeDeltas[i];
    });

    if (gridRef.current) {
      // $FlowFixMe
      var domWidth = gridRef.current.props.width;
      ...
      var padding = remainder / measuredWidths.length;

      if (padding > 0) {
        return resizedWidths.map(function (w) {
  ...
  return React.createElement(React.Fragment, null, React.createElement(_measureColumnWidths.default, {
    ...
    , {
      ...
      height: height // plus one to account for additional header row
      ,
